...
"Usaria HTTP sincrono con un timeout generoso — digamos 15 segundos para OCR. El orquestador ya ejecuta pasos independientes en paralelo, entonces mientras OCR es lento, Liveness corre al mismo tiempo. Si OCR consistentemente excede el timeout, agregaria logica de reintento para ese paso especifico. Si el problema persiste, ahi consideraria mover OCR a asincrono con callback, pero solo para ese servicio."
